...
string orb_result_path = "../CameraTrajectory.txt";

void Draw(const VecMatrix4d &poses, const VecVec3d &points);

int main()
{
    fstream orb_result_file;
    orb_result_file.open(orb_result_path, ios::app | ios::in);
    ...
    while(!orb_result_file.eof())
    {
        Matrix4d cur_pose = Eigen::Matrix4d::Identity();
        orb_result_file >> cur_pose(0,0) >> cur_pose(0,1) >> cur_pose(0,2) >> cur_pose(0,3)
        ...
        // Sophus::SE3d tmp(cur_pose);
        // plot_poses.push_back(Sophus::SE3d(cur_pose));
    }
